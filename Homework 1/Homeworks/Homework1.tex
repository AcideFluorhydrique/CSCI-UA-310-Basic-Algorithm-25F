...
(3 points each) For each of the following functions $f, g$, state whether $f(n) = O(g(n)), f(n) = \Omega(g(n)),$ or $f(n) = \Theta(g(n))$. Justify your answers in 1-2 sentences (i.e., by using one of the methods for proving asymptotics, the definitions or the limit definitions). 
...
\begin{enumerate}
    \item (1 point each) For each of the following scenarios, determine the Big-O complexity. (There is one each of $O(1)$, $O(n)$, $O(n^2)$, and $O(2^n)$).
        \begin{enumerate}
            ...
            \item Eating one chip out of a bag of $n$ chips.
